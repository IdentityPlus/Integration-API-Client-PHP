...
class Identity_Plus_API {
    ...
    const validation_endpoint = "https://my.identity.plus";
    ...
    public function anonymous_id_retrival_endpoint($return_url = NULL){
    	...
    	$request = new Redirect_Request($return_url);
        $sb = self::validation_endpoint."/anonymous-id?api=".$this->cert_details['serialNumber']."&payload=";

        $request = $request->to_json();
		$encrypted = $this->encrypt($request);
        $encoded = Identity_Plus_Utils::base64url_encode($encrypted);
    ...
    public function certificate_validation_endpoint($return_url = NULL){
    	...
    	$request = new Redirect_Request($return_url);
    	$sb = self::validation_endpoint."?api=".$this->cert_details['serialNumber']."&payload=";
